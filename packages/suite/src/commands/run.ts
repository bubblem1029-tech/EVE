...
export async function run(options: RunOptions): Promise<RunResult> {
  ...
  // ── 生成 report-data.json：在 Playwright 进程结束后统一生成，避免 keve-reporter 的 onEnd 与 JSON reporter 的 race condition ──
  // keve-reporter.ts 的 onEnd 在 Playwright 进程内部执行，此时 JSON reporter 可能尚未完全刷新 test-results.json，
  // 导致 generateReportData 读取到旧数据（specs 为空）→ 所有 case 被标记为 missing。
  // 因此在 run() 外部（Playwright 进程结束后）统一重新生成，确保 test-results.json 已完全落盘。
  const reportDataPath = path.join(resultDir, 'report-data.json');
  try {
    const confidencePath = path.join(resultDir, 'confidence-data.jsonl');
    const casesPath = path.join(taskDir, 'cases', 'test-cases.yaml');
    const reportData = await generateReportData({
      projectRoot: taskDir,
      resultsPath: resultsFile,
      confidencePath: fs.existsSync(confidencePath) ? confidencePath : undefined,
      casesPath: fs.existsSync(casesPath) ? casesPath : undefined,
    });
    fs.writeFileSync(reportDataPath, JSON.stringify(reportData, null, 2));
    console.log(chalk.green(`  ✓ report-data.json generated at: ${reportDataPath}`));
  } catch (err: any) {
    console.error(chalk.red(`  ✗ report-data.json generation failed: ${err.message}`));
    // 主生成失败时，若 report-data.json 仍不存在，尝试生成极简 fallback
    if (!fs.existsSync(reportDataPath)) {
      try {
        ...
        console.error(chalk.red(`  ✗ Fallback report generation failed: ${fallbackErr.message}`));
      }
    }
  }
